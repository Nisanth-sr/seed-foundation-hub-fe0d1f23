import { BIG_FIVE_QUESTIONS, RIASEC_QUESTIONS } from "./career-questions";
import {
  hollandCode,
  level,
  matchCareers,
...
export function buildAnalysisPrompt(payload: AnalysisPayload): string {
  ...
  const matchLines = matches.map((m) => `${m.career.title_en} (${m.score}% match): ${m.reasons.join("; ")}`);
...
export function parseAnalysisResponse(raw: string): AnalysisResult {
  ...
  parsed.disclaimer = parsed.disclaimer || "For career exploration only — not a psychological diagnosis.";
  return parsed;
}
