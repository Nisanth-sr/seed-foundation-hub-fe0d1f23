...
function ResultsPageContent() {
  ...
  const type = typeParam === "bigfive" || typeParam === "riasec" ? typeParam : undefined;
    ...
    <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6">
      <div className="mb-6 flex items-center justify-between">
        <Button variant="ghost" size="sm" asChild>
          <Link href="/career/dashboard"><ArrowLeft className="mr-1 h-4 w-4" /> {t("results.back")}</Link>
        </Button>
        <div className="flex gap-2">
          <Button variant={activeType === "bigfive" ? "primary" : "outline"} size="sm" onClick={() => router.push("/career/results?type=bigfive")}>
            {t("dash.bigfive")}
          </Button>
          <Button variant={activeType === "riasec" ? "primary" : "outline"} size="sm" onClick={() => router.push("/career/results?type=riasec")}>
            {t("dash.riasec")}
          </Button>
        </div>
      </div>
      ...
      {activeType === "bigfive" ? (
        <BigFiveResults scores={bfScores} onRetake={() => router.push("/career/assessment/bigfive")} />
      ) : (
        <RiasecResults scores={riScores} onRetake={() => router.push("/career/assessment/riasec")} />
      )}
    </div>
  );
}
...
export default function ResultsPage() {
  return (
    <Suspense fallback={<div className="flex min-h-[50dvh] items-center justify-center"><Loader2 className="h-6 w-6 animate-spin" /></div>}>
      <ResultsPageContent />
    </Suspense>
  );
}
