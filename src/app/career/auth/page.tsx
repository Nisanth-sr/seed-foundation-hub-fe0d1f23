...
function AuthPageContent() {
  ...
  useEffect(() => {
    if (!loading && user) router.push("/career/dashboard");
  }, [user, loading, router]);
  ...
  async function onSignIn(e: React.FormEvent) {
    ...
    else router.push("/career/dashboard");
  }
  ...
  async function onGoogle() {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/auth/callback?next=/career/dashboard`,
      },
    });
    if (error) {
      setBusy(false);
      ...
      <Card className="w-full">
        ...
        <CardContent>
          ...
          <div className="mt-6 text-center text-xs text-muted-foreground">
            <Link href="/career" className="hover:text-foreground">← {t("nav.home")}</Link>
          </div>
        </CardContent>
      </Card>
    </div>
...
export default function AuthPage() {
  return (
    <Suspense fallback={<div className="flex min-h-[50dvh] items-center justify-center text-muted-foreground">Loading…</div>}>
      <AuthPageContent />
    </Suspense>
  );
}
